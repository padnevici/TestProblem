...
For $i = 0 To 6
	$ahPart[$i] = _WinAPI_LoadImage(0, @ScriptDir & '\Extras\Mice' & $i & '.bmp', $IMAGE_BITMAP, 0, 0, $LR_LOADFROMFILE)
Next

; Copy some bitmaps for proper "Mice" drawing
$ahPart[7] = _WinAPI_CopyBitmap($ahPart[0])
$ahPart[8] = _WinAPI_CopyBitmap($ahPart[6])
...
Local $idPic = GUICtrlCreatePic('', 0, 0, 160, 160)
...
Func _SetBitmap($hWnd, $hBitmap, $bUpdate = 0)
		...
		Case $g_hForm
			...
			If _WinAPI_GetRawInputData($lParam, DllStructGetPtr($tRIM), DllStructGetSize($tRIM), $RID_INPUT) Then
				...
				If BitAND($iFlags, BitOR($RI_MOUSE_MIDDLE_BUTTON_DOWN, $RI_MOUSE_MIDDLE_BUTTON_UP, $RI_MOUSE_LEFT_BUTTON_DOWN, $RI_MOUSE_LEFT_BUTTON_UP, $RI_MOUSE_RIGHT_BUTTON_DOWN, $RI_MOUSE_RIGHT_BUTTON_UP)) Then
					...
					Local $hSv = _WinAPI_SelectObject($hDC, $ahPart[8])
					Select
						Case BitAND($iFlags, $RI_MOUSE_MIDDLE_BUTTON_DOWN)
							_WinAPI_DrawBitmap($hDC, 0, 0, $ahPart[5])
						Case BitAND($iFlags, $RI_MOUSE_MIDDLE_BUTTON_UP)
							_WinAPI_DrawBitmap($hDC, 0, 0, $ahPart[6])
					EndSelect
					_WinAPI_SelectObject($hDC, $ahPart[7])
					Select
						Case BitAND($iFlags, $RI_MOUSE_LEFT_BUTTON_DOWN)
							_WinAPI_DrawBitmap($hDC, 39, 25, $ahPart[1])
						Case BitAND($iFlags, $RI_MOUSE_LEFT_BUTTON_UP)
							_WinAPI_DrawBitmap($hDC, 39, 25, $ahPart[2])
						Case BitAND($iFlags, $RI_MOUSE_RIGHT_BUTTON_DOWN)
							_WinAPI_DrawBitmap($hDC, 81, 25, $ahPart[3])
						Case BitAND($iFlags, $RI_MOUSE_RIGHT_BUTTON_UP)
							_WinAPI_DrawBitmap($hDC, 81, 25, $ahPart[4])
					EndSelect
					_WinAPI_DrawBitmap($hDC, 74, 33, $ahPart[8])
					_WinAPI_SelectObject($hDC, $hSv)
					_WinAPI_DeleteDC($hDC)
					_SetBitmap($idPic, $ahPart[7])
				EndIf
